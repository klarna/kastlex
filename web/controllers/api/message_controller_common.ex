defmodule Kastlex.API.Common do
  require Logger
  require Record
      ...
      case resolve_offset(pid, topic, partition, orig_offset) do
        {:ok, offset} ->
          ...
          fetch_opts = %{max_wait_time: max_wait_time,
                         min_bytes: min_bytes,
                         max_bytes: max_bytes}
          # make a fetch request, :brod_kafka_request.fetch/1 should resolve
          # request API version from the connection pid
  ...
  defp handle_fetch_response({:ok, {hw_offset, messages}}, type) do
    resp = case type do
      ...
      "binary" ->
        ...
        %{headers: message |> kafka_message(:headers) |> Poison.encode,
